...
export const list: RequestHandler = async (req, res) => {
	try {
		...
		res.status(200).json({
			data: messages,
			cursor: messages.length !== 0 && messages.length === Number(limit) ? messages[messages.length - 1].pkId : null,
		});
	} catch (e) {
...
export const send: RequestHandler = async (req, res) => {
	try {
		...
		const message = 'An error occured during message';
		logger.error(e, message);
		res.status(500).json({ error: message });
	}
};
...
export const sendBulk: RequestHandler = async (req, res) => {
	...
	res.status(req.body.length !== 0 && errors.length === req.body.length ? 500 : 200).json({ results, errors });
};
...
export const presence: RequestHandler = async (req, res) => {
	try {
		...
		const exists = await jidExists(session, jid, type);
		if (!exists) return res.status(400).json({ error: 'JID does not exists' });
		const result = await session.sendMessage(jid, templateMessage);
		console.log(result);
		res.status(200).json(result);
	} catch (e) {
		const message = 'An error occured during presence';
		logger.error(e, message);
		res.status(500).json({ error: message });
	}
};
